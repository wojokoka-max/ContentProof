...
BEGIN
  ...
  ELSE
    ...
    WHERE subject_id = p_subject_id AND bucket_key = 'free:starter'
    FOR UPDATE;
  ...
  UPDATE analysis_quota_buckets
  SET used = used + 1, updated_at = now()
  WHERE subject_id = p_subject_id
    AND bucket_key = v_bucket_key
    AND used < limit_value
  RETURNING used, analysis_quota_buckets.limit_value
    INTO v_used, v_limit;
...
BEGIN
  ...
  IF v_bucket_key IS NOT NULL THEN
    ...
    WHERE subject_id = p_subject_id AND bucket_key = v_bucket_key;
  END IF;
END;
$$;
