...
test('expands a short FAQ answer only with supporting evidence from the current article', () => {
  ...
  expect(source).toContain('faqEvidenceKeys(`${question} ${answer}`)');
  expect(source).toContain('const preparedAnswer = item.answer');
  expect(source).toContain('const answer = buildPublishableFaqAnswer(content, question, preparedAnswer)');
});
