...
const confirmation = fs.readFileSync(
  path.join(root, 'app', 'api', 'billing', 'confirm', 'route.ts'),
  'utf8'
);
const cancellation = fs.readFileSync(
  path.join(root, 'app', 'api', 'billing', 'cancel', 'route.ts'),
  'utf8'
);
...
test('checkout requires a signed-in user and server-selected prices', () => {
  ...
  expect(checkout).toContain('access.isAdmin && !creditPurchase');
  expect(checkout).toContain('stripe.customers.retrieve(customerId)');
  expect(checkout).toContain("candidate.code === 'resource_missing'");
  expect(checkout).toContain("console.error('[/api/billing/checkout]'");
  expect(pricingSource).toContain('readJsonResponse(response)');
});
...
test('webhook processing is idempotent and subscription records are unique', () => {
  ...
  expect(webhook).toContain('recordCreditPurchase(session)');
});

test('credit confirmation verifies session ownership and refreshes the balance', () => {
  expect(confirmation).toContain("sessionId.startsWith('cs_')");
  expect(confirmation).toContain('ownerId !== access.userId');
  expect(confirmation).toContain("session.payment_status !== 'paid'");
  expect(confirmation).toContain('recordCreditPurchase(session)');
  expect(confirmation).toContain("bucket_key = 'credits:purchased'");
  expect(pricingSource).toContain('Potwierdzamy płatność i dodajemy kredyty');
});
...
test('subscription cancellation is owner-scoped and keeps access until period end', () => {
  expect(cancellation).toContain('!access.signedIn || !access.userId');
  expect(cancellation).toContain('getBillingAccess(access.userId)');
  expect(cancellation).toContain('billing.stripeSubscriptionId');
  expect(cancellation).toContain('{ cancel_at_period_end: true }');
  expect(cancellation).toContain('AND owner_id = ${access.userId}');
  expect(pricingSource).toContain('Premium pozostanie aktywne do końca opłaconego okresu');
  expect(pricingSource).toContain('Anuluj odnowienie');
});
