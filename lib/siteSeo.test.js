...
const instructionsLayout = fs.readFileSync(path.join(root, 'app', 'instrukcja', 'layout.tsx'), 'utf8');
const instructionsPage = fs.readFileSync(path.join(root, 'app', 'instrukcja', 'page.tsx'), 'utf8');
...
const crawler = fs.readFileSync(path.join(root, 'lib', 'crawler.ts'), 'utf8');
...
test('provides canonical and social metadata for public pages', () => {
  ...
  expectIncludes(instructionsLayout, "canonical: '/instrukcja'");
  expectIncludes(instructionsLayout, 'index: true');
  expectIncludes(instructionsLayout, 'openGraph:');
  expectIncludes(pricingLayout, 'robots:');
  expectIncludes(pricingLayout, 'index: true');
});
...
test('publishes robots and sitemap metadata routes', () => {
  ...
  expectIncludes(sitemap, '`${SITE_URL}/instrukcja`');
  expectIncludes(sitemap, 'lastModified');
});

test('keeps private account resources out of crawler-rendered public pages', () => {
  expectIncludes(crawler, 'googlebot');
  expectIncludes(layout, "headers().get('user-agent')");
  expectIncludes(layout, '!isCrawlerUserAgent(userAgent)');
  expectIncludes(homePage, 'accountAuthEnabled');
  expectIncludes(homePage, 'setIsCrawlerClient(isCrawlerUserAgent(navigator.userAgent))');
  expectIncludes(homePage, 'if (!accountAuthEnabled) return;');
  expectIncludes(homePage, '<AccountControls');
  expectIncludes(homePage, 'authEnabled={accountAuthEnabled}');
});
...
test('describes all three analysis modes on the public home page', () => {
  ...
  expectIncludes(homePage, 'href="/instrukcja"');
});

test('publishes a practical ContentProof instruction page', () => {
  expectIncludes(instructionsPage, "'@type': 'HowTo'");
  expectIncludes(instructionsPage, 'Wybierz odpowiedni tryb');
  expectIncludes(instructionsPage, 'SEO Pack');
  expectIncludes(footer, 'href="/instrukcja"');
});

test('publishes semantic homepage content for users and search engines', () => {
  expectIncludes(homePage, 'SeoLandingContent');
  expectIncludes(homePage, 'meta title');
  expectIncludes(homePage, 'meta description');
  expectIncludes(homePage, 'canonical URL');
  expectIncludes(homePage, 'Open Graph');
  expectIncludes(homePage, 'Twitter Card');
  expectIncludes(homePage, 'schema.org');
  expectIncludes(homePage, 'ContentEcosystemAnalyzer');
  expectIncludes(homePage, 'NextDoor Studio');
});

test('publishes structured pricing data for ContentProof offers', () => {
  expectIncludes(pricingLayout, "'@type': 'WebPage'");
  expectIncludes(pricingLayout, 'Cennik ContentProof');
  expectIncludes(pricingLayout, "'@type': 'SoftwareApplication'");
  expectIncludes(pricingLayout, "'@type': 'Offer'");
  expectIncludes(pricingLayout, "price: '49'");
  expectIncludes(pricingLayout, "price: '399'");
});
