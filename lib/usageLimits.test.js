...
test('plans have the approved limits', () => {
  expect(creditMigration).toContain("v_bucket_key := 'guest:lifetime'");
  expect(creditMigration).toContain('v_limit := 1');
  expect(creditMigration).toContain('v_limit := 5');
  expect(creditMigration).toContain("p_billing_period = 'yearly' THEN 200 ELSE 30");
  expect(usageSource).toContain("access.billingPeriod === 'yearly' ? 200 : 30");
});

test('monthly credits are consumed before purchased credits', () => {
  const monthlyUpdate = creditMigration.indexOf('quota_bucket.used + 1');
  const purchasedUpdate = creditMigration.indexOf('credit_bucket.used + 1');
  if (monthlyUpdate < 0 || purchasedUpdate < 0 || monthlyUpdate >= purchasedUpdate) {
    throw new Error('Expected monthly quota to be reserved before purchased credits');
  }
  expect(creditMigration).toContain("'credits:purchased'");
  expect(creditMigration).toContain("p_plan <> 'guest'");
});
...
test('quota SQL avoids both ambiguous columns and database-specific constraint names', () => {
  expect(quotaConflictFixMigration).toContain('ON CONFLICT DO NOTHING');
  expect(quotaConflictFixMigration).notToContain(
    'ON CONFLICT (subject_id, bucket_key) DO NOTHING'
  );
  expect(quotaConflictFixMigration).notToContain('analysis_quota_buckets_pkey');
});
