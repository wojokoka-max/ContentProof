...
test('seo generator rejects list-like fragments as meta description source', () => {
  ...
  expect(source).toContain('!isListLikeText(candidate.text)');
  expect(source).toContain('buildDescriptionFromContent');
});

test('seo generator rejects procedural steps as title fallbacks', () => {
  expect(source).toContain('isProceduralInstruction');
  expect(source).toContain('isUsableTitleSource');
  expect(source).notToContain('|| safeTextCandidate(content.plainText)');
});
