...
test('generated meta description ends as a complete sentence', () => {
  const text = `Ciasto truskawkowe na mace orkiszowej
...
- jak zrobic bardziej deserowa wersje ciasta.

Blok kontekstu

Maka orkiszowa daje bardziej delikatny i lekko orzechowy smak niz klasyczna maka pszenna. W polaczeniu z jogurtem i maslem dobrze utrzymuje wilgoc, dzieki czemu ciasto pozostaje miekkie takze nastepnego dnia.`;

  const result = analyze(text, 'text', 'meta-complete-sentence-test');
  assert(/[.!?]$/.test(result.seoPack.metaDescription), `Description should end with punctuation: ${result.seoPack.metaDescription}`);
  assert(!/\b(z|ze|w|we|na|do|dla|po|przy|i|oraz|ale)$/i.test(result.seoPack.metaDescription), `Description ends with dangling word: ${result.seoPack.metaDescription}`);
});
