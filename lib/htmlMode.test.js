...
test('complete HTML combines page-builder text blocks instead of reading only one widget', () => {
  const source = fs.readFileSync(path.join(__dirname, 'parser', 'htmlParser.ts'), 'utf8');
  expect(source).toContain('const textBlockCandidates = collectCandidates');
  expect(source).toContain("textBlockCandidates.map(candidate => candidate.html).join('\\n')");
  expect(source).toContain('combinedTextLength > Math.max(300, bestSingleTextBlockLength * 1.25)');
});

test('plain text word count is based on the full pasted article', () => {
  const source = fs.readFileSync(path.join(__dirname, 'parser', 'htmlParser.ts'), 'utf8');
  expect(source).toContain('const plainText = raw.trim()');
  expect(source).toContain('const wordCount = countWords(plainText)');
});
