...
test('Polish plain-text FAQ heading is detected and included in schema', () => {
  const text = `Mrożony napój rabarbarowo-hibiskusowy low carb

Mrożony napój rabarbarowo-hibiskusowy low carb ma intensywnie rubinowy kolor, lekko owocowy smak i delikatnie winny aromat. To prosty napój na upalne dni, który można podać bardzo mocno schłodzony.

Najczęstsze pytania

Czy hibiskus jest bardzo kwaśny?

Tak, dlatego warto połączyć go z allulozą lub malinami. Owoce i słodzik łagodzą kwaśny charakter naparu.

Czy można zrobić wersję gazowaną?

Tak, po schłodzeniu można dodać wodę gazowaną. Najlepiej zrobić to tuż przed podaniem, żeby napój pozostał lekki.

Czy napój można zamrozić?

Tak, świetnie sprawdza się jako granita. Po zmrożeniu zachowuje intensywny kolor i mocno orzeźwiający smak.`;

  const result = analyze(text, 'text', 'faq-polish-heading-schema-test');
  assertPublishableFaq(result);
  expect(result.expansionPack.faqSuggestions.length === 3, 'Polish FAQ heading should produce exactly 3 FAQ items');
  expect(result.seoPack.jsonLd.includes('"@type": "FAQPage"'), 'Polish plain-text FAQ is missing from schema');
});
